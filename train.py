...
data_folder = "/home/iplab/final_dataset"
...
checkpoint ="pre_BEST_23checkpoint_coco_5_cap_per_img_5_min_word_freq.pth.tar"
# checkpoint = './pretrained_model/BEST_34checkpoint_coco_5_cap_per_img_5_min_word_freq.pth.tar'
#checkpoint = 'pre_NOT_BEST_36checkpoint_coco_5_cap_per_img_5_min_word_freq.pth.tar'
#checkpoint='./best_models/_BEST_35checkpoint_coco_5_cap_per_img_5_min_word_freq.pth.tar'
#checkpoint = "pre_NOT_BEST_35checkpoint_coco_5_cap_per_img_5_min_word_freq.pth.tar"
def main():
    """
    ...
    if checkpoint is None:
        ...
        decoder_optimizer = torch.optim.Adamax(params=filter(lambda p: p.requires_grad, decoder.parameters()), lr=decoder_lr ,weight_decay=1e-8)
...
def train(train_loader, decoder, criterion_ce, criterion_dis, decoder_optimizer, epoch):
    ...
    #print(filter(lambda p: p.requires_grad, decoder.parameters()))
    #print(decoder.attention.weight.grad)

    # Batches
    for i, (imgs, caps, caplens, topic, attribute) in enumerate(train_loader):
        ...
        # Forward prop.
        scores, scores_d,caps_sorted, decode_lengths, sort_ind = decoder(imgs, caps, caplens, topic, attribute)
        ...
        torch.nn.utils.clip_grad_norm_(filter(lambda p: p.requires_grad, decoder.parameters()), clip)
...
def validate(val_loader, decoder, criterion_ce, criterion_dis):
    ...
    # Batches
    with torch.no_grad(): 
        for i, (imgs, caps, caplens,allcaps, topic, attribute) in enumerate(val_loader):
