...
class CaptionDataset(Dataset):
    ...
    def __init__(self, data_folder, data_name, split, transform=None):
        ...
        # Load topics
        with open(os.path.join(data_folder, self.split + '_TOPICS_' + data_name + '.json'), 'r') as j:
            self.topics = json.load(j)
    ...
    def __getitem__(self, i):
        ...
        LDA_vctor = self.topics[i // self.cpi].strip('][').split(', ')
        LDA_vctor = [np.float32(t) for t in LDA_vctor]
    
        topic = torch.FloatTensor(LDA_vctor)
        
        if self.split is 'TRAIN':
            return img, caption, caplen, topic
        else:
            # For validation of testing, also return all 'captions_per_image' captions to find BLEU-4 score
            all_captions = torch.LongTensor(
                self.captions[((i // self.cpi) * self.cpi):(((i // self.cpi) * self.cpi) + self.cpi)])
            return img, caption, caplen,all_captions, topic
