...
class Attention(nn.Module):
    ...
    def __init__(self, features_dim, decoder_dim, attention_dim, topic_dim, attribute_dim,  dropout=0.5):
        """
        ...
        self.topic_att = weight_norm(
            nn.Linear(topic_dim, attention_dim))  # linear layer to transform topic vector
        self.attribute_att = weight_norm(
            nn.Linear(attribute_dim, attention_dim))  # linear layer to transform decoder's output

        self.weighted_att_topic = weight_norm(

            nn.Linear(attribute_dim + topic_dim, attention_dim))  # linear layer to transform decoder's output
        self.full_att = weight_norm(nn.Linear(attention_dim, 1))  # linear layer to calculate values to be softmax-ed

        self.full_att_topic_att = weight_norm(nn.Linear(attention_dim, 1))  # linear layer to calculate values to be softmax-ed

        self.relu = nn.ReLU()
        self.dropout = nn.Dropout(p=dropout)
        self.softmax = nn.Softmax(dim=1)  # softmax layer to calculate weights

    def forward(self, image_features, decoder_hidden, topic, attribute):
        """
        Forward propagation.
        ...
        att3 = self.topic_att(topic)  # (batch_size, attention_dim)
        att4 = self.attribute_att(attribute)  # (batch_size, attention_dim)

        att_weight = self.full_att_topic_att(self.dropout(self.relu(att3 + att4)))
        beta = self.softmax(att_weight)
        attention_weighted_attribute = (attribute*beta)

        weighted_att_cat_topic = torch.cat((attention_weighted_attribute, topic), 1) #(batch_size, 1079)
        att5 = self.weighted_att_topic(weighted_att_cat_topic) #(batch_size, attention_dim)

        att5_refined = att5 + att2

        att = self.full_att(self.dropout(self.relu(att1 + att5_refined.unsqueeze(1)))).squeeze(2)  # (batch_size, 36)
...
class DecoderWithAttention(nn.Module):
    ...
    def __init__(self, attention_dim, embed_dim, decoder_dim, topic_dim, attribute_dim, vocab_size, features_dim=2048, dropout=0.5):
        ...
        self.attention = Attention(features_dim, decoder_dim, attention_dim, topic_dim, attribute_dim)  # attention network
    ...
    def forward(self, image_features, encoded_captions, caption_lengths, topics, attributes):
        ...
        for t in range(max(decode_lengths)):
            ...
            attention_weighted_encoding = self.attention(image_features[:batch_size_t], h1[:batch_size_t], topics[:batch_size_t], attributes[:batch_size_t])
